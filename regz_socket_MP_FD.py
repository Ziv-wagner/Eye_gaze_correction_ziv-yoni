...
#     out = cv2.VideoWriter('zoom_vid_write.avi', fourcc, 20.0, (640, 480))
#     cv2.waitKey(100)
#     print("got 1")
...
class video_receiver:
    ...
    def start_recv(self,shared_v,lock):
        ...
        while True:
            ...
            # final_video.write_videofile('try_1.mp4')
            # cv2.waitKey(5000)
            # # global out
            # # lock.acquire()
            # # out.write(frame)
            ...
            self.counter+=1
            cv2.imwrite('multi_frames\\fr_' + str(self.counter) + '_.jpg', frame)
            lock.release()
...
class gaze_redirection_system:
    def __init__(self,shared_v,lock,ourRw):
        ...
        self.Rw = [0,0]
        self.Pe_z = -60
        self.our_Rw = ourRw # [600,300],[600,700],[1300,300],[1300,700]
        #### get configurations
        self.f = conf.f
    ...
    def shifting_angles_estimator(self, R_le, R_re,shared_v,lock):
        ...
        try:
            ...
            pos_remote_head = (int(size_window[0]/2),int(size_window[1]/2))
        # self.our_Rw  # [1300,700] #changed-
        R_w = self.our_Rw  # [1300,700] #changed-(Rw_lt[0]+pos_remote_head[0], Rw_lt[1]+pos_remote_head[1])
        ...
        return alpha, self.Pe, R_w # [600,300],[600,700],[1300,300],[1300,700]    R_w
    ...
    def flx_gaze(self, frame, gray, detections, shared_v, lock, pixel_cut=[3,4], size_I = [48,64]):
        ...
        p_e=[0,0]
        R_w=[0,0]
        R_w= self.our_Rw # [1300,700] #changed
        for k,bx in enumerate(detections):
...
#     vs_thread.start()
#     time.sleep(1)
#     gz_thread = mp.Process(target=gaze_redirection_system, args=(v,l,[600,300])) # [600,300],[600,700],[1300,300],[1300,700]
#     gz_thread.start()
#     vs_thread.join()
#     gz_thread.join()
#     print("got here")
